...
class I3App {
	...
	* @param {array<string|null|object>} [inputs] Array of input files, either the path to the file on disk or `null` if not specified (i.e. file is optional), if this is a URL it is downloaded as is (the file conversion may fail) otherwise use object notation to specify file details, object notation should conform to `{url, filename?}`
	...
	*/
	run = options => {
		...
		var session = { // Session specific storage, perishes after function completes
			...
			templateArgs: {}, // Computed before main run cycle with contents of templatable values
		};
		...
		return Promise.resolve()
			...
			.then(()=> {
				...
				settings.inputs.forEach((input, inputOffset) => this.log(`  Slot #${inputOffset} =`, input.url || input));
				this.log(`* ${settings.outputs.length} Output${settings.outputs.length == '1' ? '' : 's'}`);
				settings.outputs.forEach((output, outputOffset) => this.log(`  Slot #${outputOffset} =`, output));
			})
			// }}}
			// Create workspace {{{
			.then(()=> temp.mkdir({prefix: 'i3-'}).then(res => session.tempDir = res))
			// }}}
			// Download input files if they are URLs (worker.type="docker" only) {{{
			.then(()=> {
				if (this.manifest.worker.type != 'docker') return; // Skip if not docker

				return Promise.all(options.inputs.map((input, inputOffset) => {
					if (/^https?:\/\//.test(input) || _.isPlainObject(input)) {
						var streamPath; // Where we are saving the file in the workspace
						if (_.isPlainObject(input) && input.url) { // Given an object with a URL component?
							streamPath = fspath.join(session.tempDir, input.filename || `input-${inputOffset}`); // Use optional filename as file to save as
							input = input.url; // Remap input to URL
						} else if (_.isPlainObject(input)) {
							throw new Error('Unknown input file type, specify null, string or object{url}');
						} else {
							streamPath = fspath.join(session.tempDir, `input-${inputOffset}`);
						}

						return Promise.resolve()
							.then(()=> this.log('Downloading input file', this.log.colors.cyan(`#${inputOffset}`), 'from', this.log.colors.cyan(input), '->', this.log.colors.cyan(streamPath)))
							.then(()=> axios({
								method: 'get',
								url: input,
								responseType: 'stream',
							...
							.then(res => new Promise((resolve, reject) => res.data.pipe(
								fs.createWriteStream(streamPath)
									.on('close', ()=> {
										this.log('Finished downloading input file', this.log.colors.cyan(`#${inputOffset}`));
										options.inputs[inputOffset] = streamPath;
										resolve();
									})
									.on('error', e => reject('Error downloading input file', this.log.colors.cyan(`#${inputOffset}`), `- ${e.toString()}`))
							)))
						console.log('DEBUG: INPUT IS URL');
					} else { // Assume regular file path
						return;
					}
				}));
			})
			// }}}
			// Create workspace with input files (worker.type="docker" only) {{{
			.then(()=> {
				if (this.manifest.worker.type != 'docker') return; // Skip if not docker

				return Promise.all(this.manifest.inputs.map((input, inputOffset) => {
					switch (input.type) {
						case 'references':
			...
			.then(()=> {
				session.templateArgs = {
					...
					inputs: this.manifest.inputs.map((input, inputOffset) => ({
						...input,
						path: this.manifest.worker.type == 'docker' ? `${this.manifest.worker.mountData || '/data'}/${input.filename}` : false,
						url: settings.inputs[inputOffset].url,
					})),
					outputs: this.manifest.outputs.map((output, outputOffset) => ({
						...output,
						path: this.manifest.worker.type == 'docker' ? `${this.manifest.worker.mountData || '/data'}/${output.filename}` : false,
						url: settings.inputs[outputOffset].url,
					})),
				};
			})
			// }}}
			// Run Docker process (worker.type="docker" only) {{{
			.then(()=> {
			...
			// }}}
			// Compute endpoint URL (worker.type="web" only) {{{
			.then(()=> {
				// Compute the Docker arguments / environment objects {{{
				if (this.manifest.worker.type != 'web') return; // Skip if not web
				session.url = template(this.manifest.worker.url, session.templateArgs);

				this.log('Will redirect to', this.log.colors.cyan(session.url));
			})
			// }}}
			// Process outputs {{{
			.then(()=> {
				if (this.manifest.worker.type == 'docker') {
					// Collect docker outputs and move to new locations {{{
					return Promise.all(this.manifest.outputs.map((output, outputOffset) => {
					...
					// Wait on task endpoint and move files when ready {{{
					return new Promise((resolve, reject) => {
						var waitingOutputs = settings.outputs // Create array of outputs we are waiting on
							.map(outputUrl => {
								var doDownload = !/^https?:\/\//.test(outputUrl);
						...
						var checkOutputs = ()=> {
							if (waitingOutputs.every(output => output.done)) return resolve(); // Got all files

							this.log('Polling', this.log.colors.cyan(waitingOutputs.filter(o => !o.done).length), 'output URLs');
							console.log('DEBUG: DOWNLOAD CHECK LOOP');

							return Promise.all(
								waitingOutputs.map(output => {
									if (output.done) return resolve(true); // Already fetched - do nothing

									console.log('OUTPUT', output);
									this.log('Probe output URL', this.log.colors.cyan(output.url));
									this.axios.get(output.url)
										.then(res => new Promise((resolve, reject) => { // File provided - dump to disk
											var outputPath = fspath.join(session.tempDir, output.filename);
											this.log('Downloading output URL', this.log.colors.cyan(output.url), '=>', this.log.colors.cyan(outputPath));
										...
										.catch(e => {
											if (e.code == 404) return resolve(false); // File does not exist yet - mark download as unavailable and try again in next loop
											reject(e.text || `Unexpected response code "${e.code}"`);
										})
								})
							)
								.then(results => {
									setTimeout(checkOutputs, results.every(r => r === true) ? 0 : waitOutputDelay); // Skip delay if all downloads on previous cycle worked, otherwise delay before retrying
								})
