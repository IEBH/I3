...
class I3App {
	...
	*/
	init = options => {
		...
		return Promise.resolve()
			.then(()=> this.path || Promise.reject('No app.path specified'))
			.then(()=> {
				if ( // Load from Git
					...
					return Promise.resolve()
						...
						.then(exists => {
							if (exists) {
								this.log('Using locally cached Git repo', this.log.colors.cyan(gitUrl));
							} else {
			...
			.then(manifest => {
				if (settings.validate) {
					...
					return this.i3.validateManifest(manifest);
				} else {
					this.log.warn('Skipping validation of manifest');
					return manifest;
				}
			})
			.then(manifest => this.manifest = Object.freeze(manifest))
	...
	* @returns {Promise <Object>} A promise which will return if the manifest validates or throws with a string of errors if not
	*/
	validateManifest = ()=> Promise.resolve()
		.then(()=> {
			var errs = [];

			if (!this.manifest) Promise.reject('Not a JSON file');
	...
	build = ()=> Promise.resolve()
		...
		.then(()=> {
			if (this.manifest.worker.type == 'url') {
				...
				var dockerFilePath = fspath.join(fspath.dirname(this.path), 'Dockerfile');
			...
			.then(()=> Promise.all(this.manifest.inputs.map((input, inputOffset) => {
				switch (input.type) {
					case 'references':
						...
						this.log(`Converting input file #${inputOffset} from ${reflibFormat} -> ${input.format}`);
						return Promise.resolve()
							...
							.then(refs => new Promise((resolve, reject) => {
								reflib.outputFile(fspath.join(session.tempDir, input.filename), refs, input.reflib || {}, (err) => {
									...
									this.log(`Converting input file #${inputOffset} ready`);
									resolve();
								});
							}))
						// }}}
