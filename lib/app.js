				...
				return Promise.resolve()
					...
					.then(accessible => {
						if (!accessible && !output.required) {
							return this.log('Skipping absent, optional output file', this.log.colors(output.filename));
						} else if (!accessible) {
							throw new Error(`Required output file "${this.log.colors(output.filename)}" was not provided by the worker!`);
						} else {
							return new Promise((resolve, reject) => {
