...
var I3App = function I3App(i3) {
	...
	*/
	...
	/**
	* @var {string} During the build phrase this is the originally provided path to the source app, after build this should be the path on disk
	*/
	app.path;
		...
		return Promise.resolve()
			...
			.then(()=> {
				if ( // Load from Git
					...
					var gitUrl = app.path;
					app.path = i3.settings.apps.cachePath
						+ '/'
						+ i3.settings.apps.appHasher(gitUrl);

					return Promise.resolve()
						.then(()=> fs.promises.mkdir(fspath.dirname(app.path), {recursive: true}))
						.then(()=> fs.promises.readdir(app.path)
							.then(()=> true)
							.catch(e => false)
						)
						.then(exists => {
							if (exists) {
								app.log('Using locally cached Git repo', app.log.colors.cyan(gitUrl));
							} else {
								// App doesn't exist - go fetch it
								app.log('Fetching Git repo', app.log.colors.cyan(gitUrl));
								...
								return exec([
									...
									.then(()=> fs.promises.rename(tempDir, app.path)) // Passed basic test - splat to real location
									.catch(e => {
										del.sync(tempDir);
										throw e;
									})
						...
						.then(()=> app.log('Fetching manifest path', app.log.colors.cyan(`${app.path}/i3.json`)))
						.then(()=> fs.promises.readFile(`${app.path}/i3.json`))
	...
	app.build = ()=> Promise.resolve()
		...
		.then(()=> {
			if (app.manifest.worker.type == 'url') {
				...
				// }}}
			} else if (app.manifest.worker.type == 'docker' && app.manifest.worker.base) {
				// type=='docker' (build using base from `app.manifest.{base,build}`) {{{
				var dockerFile = ''
				...
				app.log('Build docker image', app.log.colors.cyan(app.name), app.log.colors.gray(`(from base image "${app.manifest.worker.base}")`));
				...
				app.log('Build docker image', app.log.colors.cyan(app.name), app.log.colors.gray('(from Dockerfile)'));

				var dockerFilePath = fspath.join(fspath.dirname(app.path), 'Dockerfile');
